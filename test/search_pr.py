properties = \
    [ ( 'department'
      , [ ( 'deputy'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'deputy_gets_mail'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'description'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'manager'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'no_approval'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'nosy'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'part_of'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'valid_from'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'valid_to'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'domain_permission'
      , [ ( 'ad_domain'
          , [ 'admin', 'it' ]
          )
        , ( 'clearance_by'
          , [ 'admin', 'it' ]
          )
        , ( 'default_roles'
          , [ 'admin', 'it' ]
          )
        , ( 'roles_enabled'
          , [ 'admin', 'it' ]
          )
        , ( 'status'
          , [ 'admin', 'it' ]
          )
        , ( 'timetracking_by'
          , [ 'admin', 'it' ]
          )
        , ( 'users'
          , [ 'admin', 'it' ]
          )
        ]
      )
    , ( 'file'
      , [ ( 'content'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'type'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'infosec_level'
      , [ ( 'is_consulting'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'order'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'internal_order'
      , [ ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'order_number'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'valid'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'location'
      , [ ( 'address'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'city'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'country'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'order'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'sync_id'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'valid_from'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'valid_to'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'msg'
      , [ ( 'author'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'content'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'date'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'files'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'inreplyto'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'messageid'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'recipients'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'summary'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'type'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'o_permission'
      , [ ( 'organisation'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'user'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'org_location'
      , [ ( 'location'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'organisation'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'sync_id'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'valid_from'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'valid_to'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'organisation'
      , [ ( 'company_code'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'description'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'may_purchase'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'sync_id'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'valid_from'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'valid_to'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'part_of_budget'
      , [ ( 'description'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'order'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'payment_type'
      , [ ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'need_approval'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'order'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'pg_category'
      , [ ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'sap_ref'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'pr_approval'
      , [ ( 'by'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'date'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'deputy'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'deputy_gets_mail'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'description'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'msg'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'order'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'purchase_request'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'role'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'role_id'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'status'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'user'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'pr_approval_config'
      , [ ( 'amount'
          , ['admin', 'procurement']
          )
        , ( 'departments'
          , ['admin', 'procurement']
          )
        , ( 'if_not_in_las'
          , ['admin', 'procurement']
          )
        , ( 'infosec_amount'
          , ['admin', 'procurement']
          )
        , ( 'oob_amount'
          , ['admin', 'procurement']
          )
        , ( 'organisations'
          , ['admin', 'procurement']
          )
        , ( 'payment_type_amount'
          , ['admin', 'procurement']
          )
        , ( 'pr_ext_resource'
          , ['admin', 'procurement']
          )
        , ( 'purchase_type'
          , ['admin', 'procurement']
          )
        , ( 'role'
          , ['admin', 'procurement']
          )
        , ( 'valid'
          , ['admin', 'procurement']
          )
        ]
      )
    , ( 'pr_approval_order'
      , [ ( 'is_board'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'is_finance'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'only_nosy'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'order'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'role'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'users'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'want_no_messages'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'pr_approval_status'
      , [ ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'order'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'transitions'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'pr_currency'
      , [ ( 'description'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'exchange_rate'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'key_currency'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'max_group'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'max_team'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'min_sum'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'order'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'pr_ext_resource'
      , [ ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'pr_offer_item'
      , [ ( 'add_to_las'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'description'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'gl_account'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'index'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'infosec_level'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'internal_order'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'is_asset'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'offer_number'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'payment_type'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'pr_currency'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'pr_supplier'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'price_per_unit'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'product_group'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'psp_element'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'purchase_type'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'sap_cc'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'supplier'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'time_project'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'units'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'vat'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'pr_rating_category'
      , [ ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'order'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'pr_status'
      , [ ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'order'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'relaxed'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'transitions'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'pr_supplier'
      , [ ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'sap_ref'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'pr_supplier_rating'
      , [ ( 'organisation'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'rating'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'scope'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'supplier'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'pr_supplier_risk'
      , [ ( 'organisation'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'security_req_group'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'supplier'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'supplier_risk_category'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'product_group'
      , [ ( 'infosec_level'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'pg_category'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'sap_ref'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'security_req_group'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'psp_element'
      , [ ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'number'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'organisation'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'project'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'project_org'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'valid'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'purchase_request'
      , [ ( 'approvals'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'charge_to'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'continuous_obligation'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'contract_term'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'date_approved'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'date_ordered'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'delivery_address'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'delivery_deadline'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'department'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'files'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'frame_purchase'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'frame_purchase_end'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'gl_account'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'infosec_level'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'infosec_project'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'intended_duration'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'internal_order'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'issue_ids'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'messages'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'nosy'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'offer_items'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'organisation'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'part_of_budget'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'payment_type'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'pr_currency'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'pr_ext_resource'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'pr_justification'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'pr_risks'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'psp_element'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'purchase_risk_type'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'purchase_type'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'purchasing_agents'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'renegotiations'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'renew_until'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'requester'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'responsible'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'safety_critical'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'sap_cc'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'sap_reference'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'special_approval'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'status'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'termination_date'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'terms_conditions'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'time_project'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'title'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'total_cost'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'purchase_risk_type'
      , [ ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'order'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'purchase_security_risk'
      , [ ( 'infosec_level'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'purchase_risk_type'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'supplier_risk_category'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'purchase_type'
      , [ ( 'allow_gl_account'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'confidential'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'description'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'forced_roles'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'nosy'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'order'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'pr_forced_roles'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'pr_roles'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'pr_view_roles'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'purchasing_agents'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'roles'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'valid'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'view_roles'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'query'
      , [ ( 'klass'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'private_for'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'tmplate'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'url'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'sap_cc'
      , [ ( 'deputy'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'deputy_gets_mail'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'description'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'group_lead'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'nosy'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'organisation'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'purchasing_agents'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'responsible'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'sync_id'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'team_lead'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'valid'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'security_req_group'
      , [ ( 'is_consulting'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'supplier_risk_category'
      , [ ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'order'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'terms_conditions'
      , [ ( 'description'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'order'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'time_project'
      , [ ( 'deputy'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'deputy_gets_mail'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'description'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'group_lead'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'infosec_req'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'nosy'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'organisation'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'purchasing_agents'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'responsible'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'status'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'sync_id'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'team_lead'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'time_project_status'
      , [ ( 'active'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'description'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'sync_id'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    , ( 'user'
      , [ ( 'ad_domain'
          , ['admin', 'procurement-admin', 'user_view']
          )
        , ( 'address'
          , [ 'admin', 'board', 'ciso', 'controlling'
            , 'dom-user-edit-facility', 'dom-user-edit-gtt'
            , 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr'
            , 'hr-approval', 'it', 'it-approval', 'las', 'measurement-approval'
            , 'o-permission'
            , 'pgp', 'pr-view', 'procure-approval', 'procurement'
            , 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user'
            , 'user_view'
            ]
          )
        , ( 'alternate_addresses'
          , ['admin', 'procurement-admin', 'user_view']
          )
        , ( 'business_responsible'
          , ['admin', 'procurement-admin', 'user_view']
          )
        , ( 'clearance_by'
          , ['admin', 'procurement-admin', 'user_view']
          )
        , ( 'csv_delimiter'
          , ['admin', 'procurement-admin', 'user_view']
          )
        , ( 'guid'
          , ['admin', 'procurement-admin', 'user_view']
          )
        , ( 'hide_message_files'
          , ['admin', 'procurement-admin', 'user_view']
          )
        , ( 'password'
          , ['admin', 'procurement-admin', 'user_view']
          )
        , ( 'queries'
          , ['admin', 'procurement-admin', 'user_view']
          )
        , ( 'realname'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement'
            , 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'roles'
          , ['admin', 'procurement-admin', 'user_view']
          )
        , ( 'status'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement'
            , 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'subst_until'
          , ['admin', 'procurement-admin', 'user_view']
          )
        , ( 'substitute'
          , ['admin', 'procurement-admin', 'user_view']
          )
        , ( 'supervisor'
          , ['admin', 'procurement-admin', 'user_view']
          )
        , ( 'timezone'
          , ['admin', 'procurement-admin', 'user_view']
          )
        , ( 'username'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'want_no_messages'
          , ['admin', 'procurement-admin', 'user_view']
          )
        ]
      )
    , ( 'user_status'
      , [ ( 'description'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'is_internal'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'is_nosy'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'is_system'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'ldap_group'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'ldap_prio'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'name'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        , ( 'roles'
          , [ 'admin', 'board', 'ciso', 'controlling', 'dom-user-edit-facility', 'dom-user-edit-gtt', 'dom-user-edit-hr', 'dom-user-edit-office', 'finance', 'hr', 'hr-approval', 'it', 'it-approval'
            , 'las', 'measurement-approval', 'o-permission', 'pgp', 'pr-view', 'procure-approval', 'procurement', 'procurement-admin', 'project', 'project_view', 'quality'
            , 'subcontract', 'subcontract-org', 'training-approval', 'user', 'user_view'
            ]
          )
        ]
      )
    ]

if __name__ == '__main__' :
    for cl, props in properties :
        print (cl)
        for p in props :
            print ('    ', p)
