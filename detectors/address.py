# Copyright (C) 2004-25 Ralf Schlatterbeck. All rights reserved
...
def set_uid (db, cl, nodeid, new_values):
    if 'uid' not in new_values:
        new_values ['uid'] = str (uuid4 ())
# end def set_uid

def require_uid (db, cl, nodeid, new_values):
    if 'uid' in new_values:
        # Allow set only if it was empty previously
        if cl.get (nodeid, 'uid'):
            raise Reject ('The uid value must not change')
# end def require_uid
...
def init (db):
    ...
    if adrclass:
        ...
        if 'uid' in adrclass.properties:
            adrclass.audit ("create", set_uid)
            adrclass.audit ("set",    require_uid)
    if persclass and persclass != adrclass:
        ...
        if 'uid' in persclass.properties:
            persclass.audit ("create", set_uid)
            persclass.audit ("set",    require_uid)
    if persclass:
